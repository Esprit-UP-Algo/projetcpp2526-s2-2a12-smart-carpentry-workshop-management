...
{
    // Le chatbot est maintenant créé dans createChatbotPage()
    // Cette méthode peut rester vide ou être supprimée
}
...
void MainWindow::createPages()
{
    ...
    stackedWidget->addWidget(createChatbotPage());
}
...
QWidget* MainWindow::createProductsPage()
{
    ...
    for (auto b : {addBtn,editBtn,deleteBtn,exportBtn}) {
...
void MainWindow::onAddProductClicked()
{
    ...
    dimensions->setPlaceholderText("L x l x H");
    dimensions->setMinimumHeight(35);
    QLineEdit *matUtilise=new QLineEdit(&dialog);
    matUtilise->setPlaceholderText("Bois, MDF, Verre...");
    ...
    connect(browseBtn,&QPushButton::clicked,[&dialog,image](){
        QString f=QFileDialog::getOpenFileName(&dialog,"Sélectionner une image",QDir::homePath(),"Images (*.png *.jpg *.jpeg *.bmp *.gif)");
        if(!f.isEmpty()) image->setText(f);
    });
    ...
    form->addRow("Matériaux:",matUtilise);
    ...
    if (dialog.exec()==QDialog::Accepted) {
        ...
        if (table) {
            ...
            if(!prixText.contains("EUR")) prixText+=" EUR";
...
void MainWindow::onEditProductClicked()
{
    ...
    connect(browseBtn,&QPushButton::clicked,[&dialog,imageEdit](){
        QString f=QFileDialog::getOpenFileName(&dialog,"Sélectionner une image",QDir::homePath(),"Images (*.png *.jpg *.jpeg *.bmp *.gif)");
        if(!f.isEmpty()) imageEdit->setText(f);
    });
    ...
    form->addRow("Matériaux:",matUtiliseEdit);
