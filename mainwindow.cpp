...
QWidget* MainWindow::createProjectsPage()
{
    ...
    // ────────────────────────────────────────────────
    // 1. STATISTIQUES D'AVANCEMENT (cartes en haut)
    // ────────────────────────────────────────────────
    ...
    QList<StatData> stats = {
        ...
        {"TERMINÉS CE MOIS", "8", "completed"},
        {"AVANCEMENT MOYEN", "67%", "progress"}  // Nouvelle carte
    };
    ...
    // ────────────────────────────────────────────────
    // 2. BARRE DE RECHERCHE PAR NOM
    // ────────────────────────────────────────────────
    QFrame *searchFrame = new QFrame(page);
    ...
    QLabel *searchLabel = new QLabel("Rechercher un projet :", searchFrame);
    searchLabel->setObjectName("searchLabel");
    searchLabel->setStyleSheet("color: #718096; font-size: 13px;");

    QLineEdit *searchEdit = new QLineEdit(searchFrame);
    searchEdit->setObjectName("projectSearch");
    searchEdit->setPlaceholderText("Nom du projet, client, type...");
    searchEdit->setMinimumWidth(300);
    searchEdit->setMaximumWidth(400);
    searchEdit->setFixedHeight(38);

    QPushButton *searchBtn = new QPushButton("Rechercher", searchFrame);
    searchBtn->setObjectName("searchButton");
    searchBtn->setCursor(Qt::PointingHandCursor);
    searchBtn->setFixedHeight(38);

    QPushButton *resetBtn = new QPushButton("Réinitialiser", searchFrame);
    ...
    // ────────────────────────────────────────────────
    // 3. ACTION BUTTONS (avec bouton d'export)
    // ────────────────────────────────────────────────
    ...
    QPushButton *exportBtn = new QPushButton("📄 Exporter Contrat PDF", page);  // Nouveau bouton
    ...
    // Le bouton d'export n'est pas connecté (juste visuel pour l'instant)
    // connect(exportBtn, &QPushButton::clicked, this, &MainWindow::onExportProjetClicked);
    ...
    // ────────────────────────────────────────────────
    // 4. PROJECTS TABLE (avec colonne Avancement ajoutée)
    // ────────────────────────────────────────────────
    QTableWidget *table = new QTableWidget(page);
    table->setObjectName("dataTable");
    table->setColumnCount(9);  // Augmenté à 9 colonnes
    ...
    QTableWidgetItem *avancement1 = new QTableWidgetItem("45%");
    avancement1->setForeground(QBrush(QColor("#8A9A5B")));
    avancement1->setTextAlignment(Qt::AlignCenter);
    table->setItem(0, 8, avancement1);
    ...
    QTableWidgetItem *avancement2 = new QTableWidgetItem("0%");
    avancement2->setForeground(QBrush(QColor("#f39c12")));
    avancement2->setTextAlignment(Qt::AlignCenter);
    table->setItem(1, 8, avancement2);
    ...
    QTableWidgetItem *avancement3 = new QTableWidgetItem("100%");
    avancement3->setForeground(QBrush(QColor("#3498db")));
    avancement3->setTextAlignment(Qt::AlignCenter);
    table->setItem(2, 8, avancement3);
    ...
    // Garder le double-click existant
    connect(table, &QTableWidget::cellDoubleClicked, this, [this, table](int row, int) {
        ...
        QString avancement = table->item(row, 8) ? table->item(row, 8)->text() : "0%";

        QString statutColor;
        if (statut == "En cours") statutColor = "#2ecc71";
        else if (statut == "En attente") statutColor = "#f39c12";
        else if (statut == "Terminé") statutColor = "#3498db";
        else statutColor = "#e74c3c";
        ...
        details.setText(QString(
                            ...
                            "<tr><td style='font-weight: bold; color: #4a5568;'>Avancement:</td><td style='color: #8A9A5B; font-weight: bold;'>%10</td></tr>"
                            "</table>"
                            "</div>"
                            ).arg(nomProjet, client, adresse, type, dateDebut, dateFin, statutColor, statut, budget, avancement));
...
void MainWindow::onAddButtonClicked()
{
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        if (table) {
            ...
            // S'assurer que le tableau a 9 colonnes
            ...
            // Ajouter la colonne d'avancement avec valeur par défaut
            QTableWidgetItem *avancementItem = new QTableWidgetItem("0%");
            avancementItem->setForeground(QBrush(QColor("#f39c12")));
            avancementItem->setTextAlignment(Qt::AlignCenter);
            table->setItem(row, 8, avancementItem);
...
void MainWindow::onEditButtonClicked()
{
    ...
    QString nomProjetValue = table->item(row, 6) ? table->item(row, 6)->text() : "";
    QString adresseValue = table->item(row, 7) ? table->item(row, 7)->text() : "";
    QString avancementValue = table->item(row, 8) ? table->item(row, 8)->text() : "0%";
    ...
    QLineEdit *avancement = new QLineEdit(avancementValue, &dialog);
    avancement->setPlaceholderText("0%");
    avancement->setMinimumHeight(35);
    ...
    form->addRow("Statut:", statut);
    form->addRow("Budget:", budget);
    form->addRow("Avancement:", avancement);
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        // Update avancement
        QTableWidgetItem *avancementItem = table->item(row, 8);
        QString avancementText = avancement->text();
        if (!avancementText.contains("%")) {
            avancementText = avancementText + "%";
        }
        avancementItem->setText(avancementText);

        // Color based on value
        int avancementVal = avancementText.replace("%", "").toInt();
        if (avancementVal >= 100) {
            avancementItem->setForeground(QBrush(QColor("#3498db")));
        } else if (avancementVal >= 50) {
            avancementItem->setForeground(QBrush(QColor("#8A9A5B")));
        } else if (avancementVal > 0) {
            avancementItem->setForeground(QBrush(QColor("#f39c12")));
        } else {
            avancementItem->setForeground(QBrush(QColor("#e74c3c")));
        }
...
void MainWindow::onDeleteButtonClicked()
{
    ...
    QString projectName = table->item(row, 6) ? table->item(row, 6)->text() : "Projet sans nom";
