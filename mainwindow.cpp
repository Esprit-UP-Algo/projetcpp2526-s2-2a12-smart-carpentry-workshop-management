...
void MainWindow::onAddProductClicked()
{
    ...
    idProd->setValidator(new QIntValidator(1, 999999, this));
    ...
    nomProd->setValidator(new QRegularExpressionValidator(regexNom, this));
    ...
    prixProd->setValidator(new QDoubleValidator(0, 999999, 2, this));
    ...
    QRegularExpression regexDim("^[0-9]+ x [0-9]+ x [0-9]+$");
    dimensions->setValidator(new QRegularExpressionValidator(regexDim, this));
    ...
    if (dialog.exec() == QDialog::Accepted)
    {
        if(idProd->text().isEmpty() || nomProd->text().isEmpty() || prixProd->text().isEmpty())
        {
            QMessageBox::warning(this,"Erreur","Veuillez remplir les champs obligatoires !");
            return;
        }

        bool ok;
        double prix = prixProd->text().toDouble(&ok);
        ...
        if(!p.isValid())
        {
            QMessageBox::warning(this,"Erreur","Produit invalide !");
            return;
        }

        if(ProduitDatabase::instance().addProduit(p))
        {
            QWidget *productsPage = stackedWidget->widget(4);
            QTableWidget *table = productsPage->findChild<QTableWidget*>("productsTable");
            ...
            if(table)
            {
                ...
                table->setItem(row,0,new QTableWidgetItem(p.getId()));
                table->setItem(row,1,new QTableWidgetItem(p.getNom()));
                table->setItem(row,2,new QTableWidgetItem(p.getCategorie()));
                table->setItem(row,3,new QTableWidgetItem(QString::number(p.getPrix())+" EUR"));
                table->setItem(row,4,new QTableWidgetItem(p.getDimensions()));
                table->setItem(row,5,new QTableWidgetItem(p.getMat()));
                table->setItem(row,6,new QTableWidgetItem(p.getDate().toString("dd/MM/yyyy")));
                table->setItem(row,7,new QTableWidgetItem(p.getImage()));
        ...
        {
            QMessageBox::critical(this,"Erreur","Erreur lors de l'ajout dans la base !");
        }
        if(idProd->text().isEmpty() || nomProd->text().isEmpty() || prixProd->text().isEmpty())
        {
        ...
        // ID NUMBER
        int id = idProd->text().toInt();
        if(id <= 0)
        {
            QMessageBox::warning(this,"Erreur","ID invalide !");
            return;
        }
        prix = prixProd->text().toDouble(&ok); // ✅ sans redéclaration


        // DATE
        if(dateCreation->date() > QDate::currentDate())
        {
...
void MainWindow::onEditProductClicked()
{
    ...
    {
        bool ok;
        double prix = prixProdEdit->text().toDouble(&ok);
        ...
        Produit p(
            idProdEdit->text(),
            nomProdEdit->text(),
            categorieProdEdit->currentText(),
            prix,
            dimensionsEdit->text(),
            matUtiliseEdit->text(),
            dateCreationEdit->date(),
            imageEdit->text()
            );

        if(ProduitDatabase::instance().updateProduit(p))
        {
            table->item(row,0)->setText(p.getId());
            table->item(row,1)->setText(p.getNom());
            table->item(row,2)->setText(p.getCategorie());
            table->item(row,3)->setText(QString::number(p.getPrix())+" EUR");
            table->item(row,4)->setText(p.getDimensions());
            table->item(row,5)->setText(p.getMat());
            table->item(row,6)->setText(p.getDate().toString("dd/MM/yyyy"));
            table->item(row,7)->setText(p.getImage());
...
void MainWindow::onDeleteProductClicked()
{
    ...
    if (reply == QMessageBox::Yes)
    {
        if(ProduitDatabase::instance().deleteProduit(idProd))
        {
            table->removeRow(row);
