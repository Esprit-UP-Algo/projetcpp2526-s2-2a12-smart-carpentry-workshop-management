...
public:
    ...
    static constexpr int PERM_TRANSACTIONS = 1 << 0;  // bit 0 = 1
    static constexpr int PERM_PROJET       = 1 << 1;  // bit 1 = 2
    static constexpr int PERM_PRODUIT      = 1 << 2;  // bit 2 = 4
    static constexpr int PERM_MATERIAU     = 1 << 3;  // bit 3 = 8
    static constexpr int PERM_EMPLOYE      = 1 << 4;  // bit 4 = 16
    static constexpr int PERM_ALL          = 0b11111;  // 31 = full access
    ...
    // Permission helpers
    bool hasPermission(int flag)  const { return (m_permissions & flag) != 0; }
    bool canAccessEmploye()       const { return hasPermission(PERM_EMPLOYE); }
    bool canAccessMateriau()      const { return hasPermission(PERM_MATERIAU); }
    bool canAccessProduit()       const { return hasPermission(PERM_PRODUIT); }
    bool canAccessProjet()        const { return hasPermission(PERM_PROJET); }
    bool canAccessTransactions()  const { return hasPermission(PERM_TRANSACTIONS); }
