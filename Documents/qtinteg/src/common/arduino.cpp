...
bool ArduinoHandler::connectToArduino()
{
    ...
    // ── PORT RÉSERVÉ AU KEYPAD/LCD ────────────────────────────────────────────
    // COM4 est utilisé par le bridge keypad (Connection::startBridge).
    // ArduinoHandler ne doit JAMAIS essayer ce port.
    static const QString RESERVED_PORT = "COM4";
    // ─────────────────────────────────────────────────────────────────────────
    ...
    for (const QSerialPortInfo &port : ports) {
        ...
        // ← IGNORER le port réservé au keypad
        if (port.portName().compare(RESERVED_PORT, Qt::CaseInsensitive) == 0) {
            qDebug() << "Skipping reserved port:" << port.portName() << "(keypad bridge)";
            continue;
        }
        ...
        for (const QSerialPortInfo &port : list) {
            ...
            if (m_serialPort->open(QIODevice::ReadWrite)) {
                ...
                connect(m_serialPort, &QSerialPort::readyRead,
                        this, &ArduinoHandler::onReadyRead);
                ...
                connect(m_timeoutTimer, &QTimer::timeout,
                        this, &ArduinoHandler::onTimeout);

                emit connectionStatusChanged(true);
                qDebug() << "Connected to Arduino on port:" << port.portName();
                return true;
            } else {
                qDebug() << "Failed to open port:" << port.portName()
    ...
    if (tryPorts(arduinoPorts)) return true;
    if (!arduinoPorts.isEmpty())
        qDebug() << "Arduino-specific ports found but none opened successfully.";
    if (tryPorts(otherPorts)) return true;
