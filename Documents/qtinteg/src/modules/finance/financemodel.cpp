...
QString FinanceModel::generateReference()
{
    ...
    // Find the next available reference number for today
    QSqlQuery query(db);
    query.prepare("SELECT MAX(TO_NUMBER(SUBSTR(REFERENCE, -4))) FROM TRANSACTIONS "
                  "WHERE REFERENCE LIKE :pattern AND TO_CHAR(DATE_TRAN, 'YYYYMMDD') = :date");
    query.bindValue(":pattern", QString("TRANS-%1-%").arg(dateStr));
    ...
    while (!unique) {
        reference = QString("TRANS-%1-%2").arg(dateStr).arg(nextNumber, 4, 10, QChar('0'));

        QSqlQuery checkQuery(db);
        checkQuery.prepare("SELECT COUNT(*) FROM TRANSACTIONS WHERE REFERENCE = :ref");
        ...
        if (checkQuery.exec() && checkQuery.next()) {
            ...
            // If check fails, use the incremented number
            unique = true;
        }
    }
