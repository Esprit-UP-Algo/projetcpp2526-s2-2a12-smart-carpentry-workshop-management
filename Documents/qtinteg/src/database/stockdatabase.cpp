...
bool StockDatabase::updateQuantiteByNom(const QString& nom, double quantiteGrams)
{
    QSqlDatabase db = QSqlDatabase::database(Connection::CONN_NAME);
    QSqlQuery q(db);
    q.prepare(
        "UPDATE MATERIAU "
        "SET    QUANTITE_MAT = :qte, "
        "       LAST_ORDER   = TO_DATE(:today, 'YYYY-MM-DD') "
        "WHERE  UPPER(NOM_MAT) = UPPER(:nom)"
        );
    q.bindValue(":qte",   quantiteGrams);
    q.bindValue(":today", QDate::currentDate().toString("yyyy-MM-dd"));
    q.bindValue(":nom",   nom);

    if (!q.exec()) {
        qWarning() << "[StockDatabase] updateQuantiteByNom error:"
                   << q.lastError().text();
        return false;
    }
    return q.numRowsAffected() > 0;
